...
const resolveBackendWsUrl = () => {
  const raw = (import.meta.env.VITE_BACKEND_WS_URL || "").trim();
  if (!raw) {
    return `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.hostname}:8000/ws/live`;
  }
  ...
  if (!normalized.includes("/ws/live")) {
    normalized = `${normalized.replace(/\/$/, "")}/ws/live`;
  }
  return normalized;
};

const BACKEND_WS_URL = resolveBackendWsUrl();
type BackendEvent = {
  type: string;
  message?: string;
  text?: string;
  ...
  interrupted?: boolean;
  audioChunks?: { mimeType: string; data: string }[];
  payload?: any;
};
...
export default function App() {
  const isLocalNoAuth = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  ...
  useEffect(() => {
    if (!currentSessionId || activeTab !== 'history' || isLocalNoAuth) return;
    ...
    return () => unsubscribe();
  }, [currentSessionId, activeTab, isLocalNoAuth]);

  const saveMessage = async (role: 'user' | 'model', text: string) => {
    if (!currentSessionId || !text.trim() || isLocalNoAuth) return;
  ...
  const startVisionLoop = useCallback(() => {
    if (!isConnected || !isCameraOn || !videoRef.current || !sessionRef.current) return;
    if (visionIntervalRef.current) return;

    visionIntervalRef.current = window.setInterval(() => {
      const videoEl = videoRef.current;
      if (!videoEl) return;
      const frame = visionProcessor.current?.captureFrame(videoEl);
      if (frame) {
  ...
  }, [isConnected, isCameraOn, startVisionLoop, stopVisionLoop]);

  const handleMessage = useCallback((message: BackendEvent) => {
    if (message.type === 'backend_error') {
      setError(message.message || message.text || message.payload?.message || 'Backend error');
      setIsThinking(false);
      return;
    }
    if (message.type !== 'gemini_event') return;

    if (Array.isArray(message.audioChunks) && audioPlayer.current) {
      message.audioChunks.forEach((chunk) => {
        if (chunk?.data) {
          audioPlayer.current?.playChunk(chunk.data);
        }
      });
    }

    const newAiText = (message.text || "").trim();
    if (newAiText) {
      setIsThinking(false);
  ...
  const createBackendSession = (userId: string): Promise<BackendSession> => {
    return new Promise((resolve, reject) => {
      const ws = new WebSocket(`${BACKEND_WS_URL}?userId=${encodeURIComponent(userId)}&audio=true`);
      let opened = false;
      ...
      ws.onopen = () => {
        ...
        resolve({
          sendRealtimeInput: (input) => {
            ...
            if (input.text) {
              ws.send(JSON.stringify({ type: 'text', text: input.text }));
              return;
            }
            if (input.media?.data && input.media?.mimeType) {
              const type = input.media.mimeType.startsWith('audio/') ? 'audio' : 'image';
  ...
  const handleLogin = async () => {
    try {
      ...
      if (code === "auth/unauthorized-domain") {
        setError("Login blocked: add localhost to Firebase Authentication > Settings > Authorized domains.");
        return;
      }
      if (code === "auth/operation-not-allowed") {
        setError("Login blocked: enable Google provider in Firebase Authentication > Sign-in method.");
        return;
      }
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const userId = user?.uid || "local-dev";
      const session = await createBackendSession(userId);
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-white/5">
                {user ? (
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 bg-zinc-800 rounded-xl">
                    ...
                    <span className="text-xs font-bold text-zinc-200">Local Mode</span>
                  </div>
                )}
              </div>
                ...
                <div className="p-8 bg-zinc-900/50 border border-white/5 rounded-3xl h-full flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 pr-2 scrollbar-hide">
                    {historicalMessages.map((msg, index) => (
                      <div key={msg.id || `message-${index}`} className={`space-y-1 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
